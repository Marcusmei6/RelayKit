...
- GUI route proof input must be bound to and re-verified against the exact isolated Desktop PID/window before every automated action. If proof text appears in the primary development thread, classify the run as failed targeting and require fresh isolated usage plus process-bound screenshots; never reuse it as route evidence.
