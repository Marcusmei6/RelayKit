...
Status: native proxy/startup/headerless SSE/mixed history implemented; build 8 passed installed same-thread switching but exposed headerless compact JSON; build 9 packaging and current-package compact validation pending.
...
- Remaining gates: commit the headerless compact fix, produce fresh Developer-ID/notarized v0.1.5 build 9 in the Mini GUI Keychain session, atomically replace invalidated build 8, then rerun the explicit current-package /v1/responses/compact request. Installed build 8 already passed the fresh same-thread provider -> official -> provider -> official marker sequence. Global config/auth may only receive before/after hash guards; never touch port 18787.
