...
Status: build 10 passed real reopened-long-thread provider -> official compaction and continuation but generated an incompatible catalog; build 11 fixed the schema but was withheld after pre-install startup fallback review; installed-CLI fallback implemented, build 12 exposed an ordinary-launch Official auth race and was rolled back; build 13 fixed startup ordering and exposed Codex 0.142.3 not retrying adapter compaction errors; direct Official compaction fallback is implemented, build 14 packaging and multi-switch validation pending.
