...
Status: source implementation is committed through `cbee713`; focused/full source validation, the random-port eight-case fault matrix, all six hosted checks, and the five-case live isolated launchd proof pass. Independent Test/CR, Build 18, and installed same-Codex E2E remain pending.
...
- The App takes an exclusive advisory lease on the existing 0600 control-token file before reading route state or registering the service. A helper replacement that sees the live lease preserves the enabled route for App adoption. A launchd helper with no owner holds its probe lease across startup reconciliation, restores stale managed fields, and only then serves, so the App and helper cannot concurrently adopt different epochs and a new login epoch does not wait on the adoption timer.
- Short idle and WebSocket disconnect are not retirement signals. The fallback service is observable and controlled by launchd; logout removes both the GUI-domain owner lease and process, and the next launchd start reconciles stale managed fields before accepting a client.
...
- Hosted run `30954732558` proves `graceful_release`, `app_loss`, `unowned_restart`, `helper_crash`, and `app_helper_loss` on clean product commit `cbee713`, including cached requests, pre-listener unowned restoration/new-client direct routing, helper restart, global guards, and cleanup. Fast Gates `30954732569`, Protocol Contract `30954732537`, and macOS App `30954732538` passed on the same SHA. `cbee713` also closes the startup adoption race by acquiring the App owner lease before reading route state and retaining the helper's no-owner lease until reconciliation completes.
