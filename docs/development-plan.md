...
- Remaining gates: run the prepared v0.1.5 Developer-ID signing/notarization command in the Mini GUI login Keychain session, run `RelayKitAppValidationTests` there, perform an exact-path controlled install of that immutable signed zip, then execute one current-package same-thread provider -> official -> provider -> official sequence that reaches compaction. Global config/auth may only receive the separately authorized non-content before/after guard; never touch port `18787`. SSH cannot perform the distribution step: `notarytool` reports the default Keychain locked and a temporary Developer-ID sign fails with `errSecInternalComponent`.
