...
Status: build 10 passed real reopened-long-thread provider -> official compaction and continuation but generated an incompatible catalog; build 11 fixed the schema but was withheld after pre-install startup fallback review; installed-CLI fallback implemented, build 12 packaging and two-provider picker validation pending.
...
- Provider-to-official remote-compaction-v2 fallback: pre-turn compaction on a model switch starts with the previous model. Adapter-backed providers cannot return the reserved encrypted compaction item, so RelayKit now rejects compaction_trigger locally with InvalidRequest and lets Codex retry with the incoming official model. Native Responses providers are not intercepted. Focused RED/GREEN plus a copied real failing long-thread compact/follow-up source-helper run pass.
...
- Remaining gates: commit the adapter compaction fallback fix, produce fresh Developer-ID/notarized v0.1.5 build 10, atomically replace invalidated build 9, then reopen the original long Desktop thread and verify natural provider -> official remote-compaction-v2 plus post-compaction continuation/model switching. Global config/auth may only receive before/after hash guards; never touch port 18787.
