...
Status: build 10 passed real reopened-long-thread provider -> official compaction and continuation, but its generated model catalog is rejected by the active Codex schema; compatibility fix implemented, build 11 packaging and two-provider picker validation pending.
