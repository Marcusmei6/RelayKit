...
Status: native proxy and startup compatibility implemented; build 6 live validation exposed and source-fixed headerless official SSE compatibility; build 7 packaging and current-package same-thread/remote-compaction validation pending.
...
- Remaining gates: commit the headerless-SSE fix, produce fresh Developer-ID/notarized v0.1.5 build 7 in the Mini GUI Keychain session, atomically replace invalidated build 6, then rerun one current-package same-thread provider -> official -> provider -> official sequence. The explicit /v1/responses/compact route remains pending because Codex 0.145 manual and threshold compaction used ordinary /v1/responses in the available non-GUI harness. Global config/auth may only receive before/after hash guards; never touch port 18787.
