...
Status: native proxy/startup/headerless SSE implemented; build 7 live switching exposed and source-fixed mixed Responses assistant-history loss; build 8 packaging and current-package switching/remote-compaction validation pending.
...
- Remaining gates: commit the mixed-history fix, produce fresh Developer-ID/notarized v0.1.5 build 8 in the Mini GUI Keychain session, atomically replace invalidated build 7, then rerun one current-package same-thread provider -> official -> provider -> official sequence. The explicit /v1/responses/compact route remains pending because Codex 0.145 manual and threshold compaction used ordinary /v1/responses in the available non-GUI harness. Global config/auth may only receive before/after hash guards; never touch port 18787.
