...
Status: native proxy implementation complete; upgrade compatibility fix implemented and focused validation passed; fresh package, GUI Keychain validation, and same-thread live validation pending.
...
- A first controlled v0.1.5 install attempt was rolled back. The install harness used ambiguous LaunchServices targeting and its rollback initially left the mapped candidate process alive; exact recovery restored installed v0.1.4 and its helper. Offline diagnosis then exposed the real upgrade risk above: the App still depended on a discoverable Codex binary to mark the existing isolated login connected. The auth-state fix invalidates the earlier ad-hoc package and requires one fresh package.
...
- Current Mini runtime is deliberately left on the restored installed v0.1.4 with its App-owned `19777` helper running because the real Codex config currently has RelayKit's two managed fields enabled. Do not stop it without either disabling RelayKit or immediately restoring a healthy helper.
- Remaining gates: commit the auth-state compatibility fix, build one fresh v0.1.5 package, run `RelayKitAppValidationTests` in the console GUI Keychain session, perform an exact-path controlled install, then execute one current-package same-thread provider -> official -> provider -> official sequence that reaches compaction. Global config/auth may only receive the separately authorized non-content before/after guard; never touch port `18787`. Developer-ID packaging/notarization still requires the Mini GUI login Keychain session because the SSH audit session reports the default Keychain locked.
