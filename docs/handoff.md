...
The app can be launched from the repository root with `./script/build_and_run.sh`; the script builds `gateway/bin/relay`, bundles it into `dist/RelayKitApp.app/Contents/MacOS/relay`, bundles the public demo provider config into `Contents/Resources/providers.example.json`, bundles the public Codex config example into `Contents/Resources/codex.config.example.toml`, sets `CFBundleExecutable` to the real Mach-O `RelayKitApp.bin`, and opens the app as a foreground macOS app. Codex Desktop also has a checked-in Run action at `.codex/environments/environment.toml` pointing to that script.
...
P0 menu-bar control-center regression is complete for the local public-safe scope. The primary app surface is menu-bar resident with a compact visible status item and an anchored small popover/control-center, tabs `接入` / `Usage` / `设置`, gateway state in the global header, real usage summaries or a real empty state, disabled Claude Code placeholder only, and a form-based provider modal overlay. The provider form writes the current public provider schema and stores credential references only. The replay/Kaboo-style UI repair moved gateway/port/health/models/Codex state into the global header, rebuilt `接入` around CLI/provider/model workflows, moved gateway controls out of the Connect catalog card, made `Usage` KPI/card-first instead of path-field-first, moved raw local paths behind Advanced settings, and kept unfinished controls hidden or disabled. The Settings tab now includes persisted Appearance (`System` / `Light` / `Dark`), a real macOS Launch at login row backed by `SMAppService`, and the visible gateway Start action; the switch is refreshed from macOS login-item status, and local unsigned login-item failures surface as macOS status/error rather than fake success. `./scripts/menu-bar-e2e-smoke.sh` captures UI screenshots plus evidence JSON under `dist/ui-smoke/` through `open -n dist/RelayKitApp.app --args ...` and now checks compact status-item visibility, anchored popover state, expected semantic sections, Settings state, Light appearance persistence, provider modal capture, Connect add/detail/selected states, internal gateway lifecycle exercise, and stale RelayKit-owned process cleanup; `./scripts/reference-model-coverage.sh` writes redacted local reference gateway source coverage to `dist/reference-model-coverage.json`; `./scripts/codex-e2e-smoke.sh` uses a temporary `CODEX_HOME`/`HOME`, fake local upstream, and RelayKit loopback gateway to prove Codex routes through RelayKit and produces local usage evidence under `dist/codex-e2e/`.
Connect now reads the local `agent-local-gateway` reference catalog only through `/v1/models`, groups by source/owner, shows auth-required state, and records redacted counts instead of raw private model IDs.
Post-P0 provider metadata now has a public-safe contract at `docs/spec/provider-profile-contract.md`. Gateway config loading and app validation understand `credential_ref`, `capabilities`, `routing`, `catalog`, and model `upstream_model`. Runtime auth supports legacy `auth_env`, `credential_ref.kind = "env"`, `credential_ref.kind = "keychain"`, and direct local `credential_ref.kind = "key_file"`. The app provider form can write a credential value to macOS Keychain while provider JSON stores only the Keychain item reference.
Direct replacement gateway hardening adds Codex-compatible `/v1/models.models` alongside OpenAI-style `data`, redacted `model_health` aggregate counts, bounded probe filtering for direct key-file providers, top-level Responses `output_text`, and normalized `total_tokens` derivation when upstream reports only input/output token counts.
Mac Pro/direct replacement acceptance can run `RELAYKIT_ACCEPTANCE_URL=http://127.0.0.1:18787 ./scripts/direct-replacement-check.sh` to verify the 18787 listener is not `agent-local-gateway` or a bridge process and that RelayKit serves `/healthz` plus the Codex-compatible model catalog shape. The check is read-only and does not stop bridge processes or edit Codex config.
...
- Menu-bar resident control-center shell: satisfied; current shell uses a compact AppKit `NSStatusItem` plus anchored `NSPopover`, keeps gateway state visible in the global header, launches through a Mach-O `CFBundleExecutable`, and records screenshot/evidence JSON through the UI smoke.
...
- Provider add sheet writes public provider metadata through `ProviderConfigDraftWriter`; credential references are stored as `credential_ref`, catalog/routing/capability metadata is validated, and Keychain/key-file references remain auth-blocked metadata until explicitly selected for runtime storage.
...
- App can call `/healthz` and `/v1/models`; bundled gateway models and redacted local reference catalog grouping are covered by app client/UI wiring and smoke evidence.
...
4. Use Settings Start if the bundled gateway is not running, then return to `接入` and use `重扫` to refresh the redacted local catalog.
