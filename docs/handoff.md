...
Current status is `needs work`. The single current product candidate is commit `665515f` and local ad-hoc zip SHA-256 `462d3b2bfe9a2a5a910e0c6d4091a1fd1ec67b29b0ae9fd63ce18045c3005001`; the extracted App tree SHA-256 is `dd065dffa81ceaa9f6c0eefa8b354515bf15ab792d3bd95a64379c67837afb37`. `dist/rc1-first-principles-current-run/product-artifact.json` binds those bytes, and this closeout must reuse them without rebuilding.
...
Two fresh ordinary launches of the current extracted App have passed the product-surface preflight under `dist/rc1-first-principles-current-run/launch-1/` and `launch-2/`: each had no product surface before the exact status-item action and exactly one same-PID WindowServer surface afterward, with a process-bound screenshot. The remote inspector observed one native AXPopover but no RelayKit semantic identifier; this is recorded as the preserved test limitation, not as evidence that the visible product surface was absent.

The current route stage is incomplete. The isolated provider destination remains empty; no current-candidate Keychain save, relaunch persistence, App-owned Gateway start, or isolated Desktop A/B/C request has completed. Therefore no current manifest PASS exists. The App and loopback fixture may stay running across a short user-session unlock, but only external provider/config/Keychain/gateway postconditions and fresh Desktop usage may advance this status.

The previously accepted run `rc1-native-20260716T111701Z-external-sandbox`, its zip SHA-256 `abf74744aedbe699e20b37064802d8753e40424d8886e68d2c32954eadec776a`, and its native App/Desktop/manifest evidence are historical and ineligible for the current candidate. They remain useful only as a prior bound-artifact compatibility result and must not be copied, relabeled, or mixed into current evidence.
...
Current-candidate focused validation has passed fresh Swift build and `RelayKitAppValidationTests`, Go test/race/vet/gofmt, manifest, Desktop AX, and manual-proof contract tests, public-boundary, and diff checks. Full current-artifact menu/Provider/Desktop evidence, diagnostics, final Test/CR, and release read-only inspection remain pending until the live route stage and cleanup complete.
