...
- Static TOML and ownership checks are not runtime proof. The first fresh smoke proved Planner `gpt-5.6-sol/ultra`, but nested generic children inherited that same metadata; that run was rejected rather than promoted as specialist proof. The workflow now requires `PARENT DISPATCH REQUIRED` and root-mediated exact registered-role launches.
- Fresh authoritative `turn_context` then proved Gateway/App/Worker `gpt-5.6-sol/high`, Test `gpt-5.6-luna/medium`, CR `gpt-5.6-sol/xhigh`, and Release `gpt-5.6-terra/high`. Gateway and App were the only concurrent lanes; Worker followed, then Test, CR, and Release ran sequentially. All lanes were read-only and the worktree remained clean. The current Desktop permission profile reports `danger-full-access` in child `turn_context` even for Test/CR, so this is behavioral read-only proof plus checked-in `sandbox_mode = "read-only"`, not a claim that the host enforced a read-only process sandbox.
...
- `$relaykit-desktop-query` requires caller-pinned catalog SHA-256, setup id, session id, and artifact SHA-256. The catalog's exact `relaykit_lineage` must match that current setup/session/artifact tuple, so a stale catalog cannot pass with only its own matching SHA. It supports `plain`, `markdown`, and `tool`, and returns redacted machine-readable metadata without query or response bodies.
