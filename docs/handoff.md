...
P0 menu-bar control-center work is now active. The primary app surface must be menu-bar resident with a small popover/control-center, tabs `接入` / `Usage` / `设置`, real gateway controls, real usage summaries, disabled Claude Code placeholder only, and a form-based provider add sheet. `./scripts/menu-bar-e2e-smoke.sh` captures UI screenshots under `dist/ui-smoke/`; `./scripts/reference-model-coverage.sh` writes redacted local reference gateway source coverage to `dist/reference-model-coverage.json`; temporary Codex E2E remains required before P0 can be called complete.
...
- Menu-bar resident control-center shell: in progress; first slice uses `MenuBarExtra` and keeps real gateway controls.
