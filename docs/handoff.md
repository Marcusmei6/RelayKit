...
The product candidate is committed through `aa3ddddc41665b86b1e59811696c53dd2e192e2f`. Fresh validation passes focused lifecycle tests, full Go test/race/vet/gofmt, Swift build and App validation, the eight-case random-port fault matrix, bundle verification, extracted local-package verification, signed-release contracts, CI workflow contracts, public-boundary scans, and diff checks. The RED chain was concrete: the launchd helper initially rejected its valid pre-adoption parentless state, then a same-run App+helper fault exposed launchd's default ten-second respawn throttle. `236bef6` permits a parentless helper only when launchd socket activation, an owner-only control token, and a positive unowned-recovery timer are all present; `aa3dddd` sets a one-second nonzero launchd throttle in the reviewed product plist.

Hosted run `30944489758` is the clean-commit launchd authority for `aa3dddd`: the offline contract, eight-case fault harness, and live unique-label launchd proof all passed. The live proof recorded four passing cases (`graceful_release`, `app_loss`, `helper_crash`, `app_helper_loss`), cached-client continuity before/after each relevant fault, restored config plus new-client direct routing where required, helper restart where required, unchanged global guards, and complete service/process/port cleanup. Local `dist/runtime-safety/evidence.json` remains setup evidence from the source worktree and must not be substituted for installed-product evidence. Independent Test/CR, Build 18, installation, and the same-running-Codex Official/provider/Official lifecycle E2E remain pending. Build 17 must not be installed or published as the final candidate.
