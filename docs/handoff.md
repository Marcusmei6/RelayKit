...
P0 menu-bar control-center regression is complete for the local public-safe scope. The primary app surface is menu-bar resident with a small popover/control-center, tabs `接入` / `Usage` / `设置`, real gateway controls, real usage summaries, disabled Claude Code placeholder only, and a form-based provider add sheet. The provider form writes the current public provider schema and stores credential references only. `./scripts/menu-bar-e2e-smoke.sh` captures UI screenshots under `dist/ui-smoke/`; `./scripts/reference-model-coverage.sh` writes redacted local reference gateway source coverage to `dist/reference-model-coverage.json`; `./scripts/codex-e2e-smoke.sh` uses a temporary `CODEX_HOME`/`HOME`, fake local upstream, and RelayKit loopback gateway to prove Codex routes through RelayKit and produces local usage evidence under `dist/codex-e2e/`.
...
- Menu-bar resident control-center shell: satisfied; current shell uses AppKit `NSStatusItem` plus `NSPopover` and keeps real gateway controls.
