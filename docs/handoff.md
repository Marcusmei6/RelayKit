...
- The App holds an advisory owner lease on the existing 0600 control-token file before reading route state, registering, or adopting the helper. A replacement helper preserves the managed route while that live App lease exists; without a live owner, it holds the same lease continuously through startup reconciliation, restores stale managed fields, and only then accepts a new login-session client.

The product candidate is committed through `cbee7137f58747ebc7130a26f97ebe8d76d23069`. Fresh validation passes focused lifecycle tests, full Go test/race/vet/gofmt, Swift build and App validation, the eight-case random-port fault matrix, bundle verification, extracted local-package verification, signed-release contracts, CI workflow contracts, public-boundary scans, and diff checks. The RED chain was concrete: the launchd helper initially rejected its valid pre-adoption parentless state, a same-run App+helper fault exposed launchd's default ten-second respawn throttle, and an audit showed that a fixed adoption grace could leave stale `19777` fields visible to a new login-session client. `236bef6` permits guarded parentless startup, `aa3dddd` sets a one-second nonzero respawn throttle, and `7e90c63` uses the existing owner-only control token as an App lifetime lease so only a live App delays restoration. A subsequent process-ordering test exposed one final adoption race: the helper previously released its lease probe before restoring config while the App could read enabled state before obtaining its lease. `cbee713` makes the App acquire first and keeps the helper's no-owner lease held across reconciliation, preventing the control and data epochs from splitting.

Hosted macOS Runtime Safety run `30954732558` is the clean-commit launchd authority for `cbee713`: the offline contract, eight-case fault harness, and live unique-label launchd proof all passed. The live proof recorded five passing cases (`graceful_release`, `app_loss`, `unowned_restart`, `helper_crash`, `app_helper_loss`); cached requests survived the fallback epoch, new clients used restored direct config, helper recovery passed, and global guards plus cleanup passed. Exact-HEAD Fast Gates `30954732569`, Protocol Contract `30954732537`, and macOS App `30954732538` also passed, representing all six required checks. Local `dist/runtime-safety/evidence.json` has `source_sha=0a0a8f6` because it was generated from the pre-commit worktree and remains setup evidence; it must not be substituted for the exact-HEAD hosted proof or installed-product evidence. Independent Test/CR, Build 18, installation, and the same-running-Codex Official/provider/Official lifecycle E2E remain pending. Build 17 must not be installed or published as the final candidate.
...
At the time of this pre-update snapshot, Build 15 remained immutable, historical, and ineligible, and Build 16 had not yet been built. The update immediately below and the 2026-07-28 current truth supersede that historical distribution status.
