...
- The App holds an advisory owner lease on the existing 0600 control-token file before registering or adopting the helper. A replacement helper preserves the managed route while that live App lease exists; without a live owner, it restores stale managed fields before accepting a new login-session client.

The product candidate is committed through `7e90c63a200aa1ff6043b14067704cbf40df7146`. Fresh validation passes focused lifecycle tests, full Go test/race/vet/gofmt, Swift build and App validation, the eight-case random-port fault matrix, bundle verification, extracted local-package verification, signed-release contracts, CI workflow contracts, public-boundary scans, and diff checks. The RED chain was concrete: the launchd helper initially rejected its valid pre-adoption parentless state, a same-run App+helper fault exposed launchd's default ten-second respawn throttle, and a final audit showed that a fixed adoption grace could leave stale `19777` fields visible to a new login-session client. `236bef6` permits guarded parentless startup, `aa3dddd` sets a one-second nonzero respawn throttle, and `7e90c63` uses the existing owner-only control token as an App lifetime lease so only a live App delays restoration.

Hosted macOS Runtime Safety run `30951938542` is the clean-commit launchd authority for `7e90c63`: the offline contract, eight-case fault harness, and live unique-label launchd proof all passed. The live proof recorded five passing cases (`graceful_release`, `app_loss`, `unowned_restart`, `helper_crash`, `app_helper_loss`). `unowned_restart` used a 30-second adoption grace but still restored config before the replacement listener served, while cached and new-direct requests both passed. Exact-HEAD Fast Gates `30951938497`, Protocol Contract `30951938559`, and macOS App `30951938585` also passed. Global guards and cleanup passed. Local `dist/runtime-safety/evidence.json` remains setup evidence from the source worktree and must not be substituted for installed-product evidence. Independent Test/CR, Build 18, installation, and the same-running-Codex Official/provider/Official lifecycle E2E remain pending. Build 17 must not be installed or published as the final candidate.
