...
The current product candidate is committed through `cbee7137f58747ebc7130a26f97ebe8d76d23069`. It adds a launchd socket-owned fallback data plane, typed provider rejection after Disable, provider credential clearing on owner release, helper crash recovery, App/helper lifecycle separation, guarded parentless launchd startup, a one-second nonzero respawn throttle, and an App owner lease on the existing 0600 control-token file. The App acquires the lease before reading route state, while an ownerless helper holds its probe lease across startup reconciliation. This lets helper crash recovery wait for a live App, forces an ownerless login/reboot start to restore stale managed fields before serving, and prevents concurrent control/data epoch adoption. Its random-port eight-case matrix and focused/full Go, Swift, bundle/package, signed-release contract, CI contract, public-boundary, and diff gates pass.

All six hosted checks pass for `cbee713`. In particular, macOS Runtime Safety run `30954732558` passed the offline contract, full eight-case harness, and live unique-label launchd proof. Its five live cases add `unowned_restart` to graceful release, App loss, helper crash, and simultaneous App/helper loss; cached and new-direct requests passed. Fast Gates `30954732569`, Protocol Contract `30954732537`, and macOS App `30954732538` passed on the same SHA. Independent Test/CR, Build 18, installation, and same-running-Codex E2E remain required.
