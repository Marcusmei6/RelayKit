...
The current product candidate is committed through `7e90c63a200aa1ff6043b14067704cbf40df7146`. It adds a launchd socket-owned fallback data plane, typed provider rejection after Disable, provider credential clearing on owner release, helper crash recovery, App/helper lifecycle separation, guarded parentless launchd startup, a one-second nonzero respawn throttle, and an App owner lease on the existing 0600 control-token file. The lease lets helper crash recovery wait for a live App while forcing an ownerless login/reboot start to restore stale managed fields before serving. Its random-port eight-case matrix and focused/full Go, Swift, bundle/package, signed-release contract, CI contract, public-boundary, and diff gates pass.

All six hosted checks pass for `7e90c63`. In particular, macOS Runtime Safety run `30951938542` passed the offline contract, full eight-case harness, and live unique-label launchd proof. Its five live cases add `unowned_restart` to graceful release, App loss, helper crash, and simultaneous App/helper loss. The unowned case used a 30-second adoption grace but still restored config before serving the replacement listener; cached and new-direct requests passed. Fast Gates `30951938497`, Protocol Contract `30951938559`, and macOS App `30951938585` passed on the same SHA. Independent Test/CR, Build 18, installation, and same-running-Codex E2E remain required.
