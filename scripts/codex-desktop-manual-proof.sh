...
preserve_existing_route_evidence() {
  ...
  local last_route_out="${2:-${LAST_ROUTE_OUT}}"
  [[ -f "${current_out}/evidence.json" ]] || return 0
  local usage_event_count
  ...
  rm -rf "${last_route_out}"
  mkdir -p "${last_route_out}"
  find "${current_out}" -maxdepth 1 -type f \( -name '*.json' -o -name '*.toml' -o -name '*.png' \) -exec cp {} "${last_route_out}/" \;
  if [[ -d "${current_out}/screenshots" ]]; then
    cp -R "${current_out}/screenshots" "${last_route_out}/screenshots"
  fi
  date -u +"%Y-%m-%dT%H:%M:%SZ" >"${last_route_out}/preserved-at.txt"
}

write_desktop_sandbox_profile() {
  cat >"${DESKTOP_SANDBOX_PROFILE}" <<EOF
(version 1)
(allow default)
...
(deny file-write* (subpath "${HOME}/.config/agent-local-gateway"))
EOF
}
...
validate_input_mode() {
  case "${PROOF_INPUT_MODE}" in
    manual_user_only|isolated_codex_cli_fallback) return 0 ;;
    *)
...
press_relaykit_app_ax() {
  local identifier="$1"
  local app_pid timeout
  app_pid="$(cat "${APP_PID_FILE}")"
  timeout="${RELAYKIT_AX_PRESS_TIMEOUT_SECONDS:-5}"
  case "${timeout}" in
    ''|*[!0-9]*) timeout=5 ;;
  esac
  for _ in {1..80}; do
    if /usr/bin/perl -e 'alarm shift; exec @ARGV' "${timeout}" swift - "${app_pid}" "${identifier}" <<'SWIFT'
...
launch_isolated_relaykit_app() {
  ...
  if ! press_relaykit_app_ax "gateway-start"; then
    wait_for_keychain_authorization || {
...
provider_gateway_status() {
  jq -r --arg model "${PROOF_PROVIDER_MODEL_ID}" '
...
ensure_provider_models_available_via_app() {
  fetch_gateway_models || {
    echo "RelayKit App gateway model list did not respond" >&2
    return 1
  }
...
EOF
  ...
  fetch_gateway_models || {
    echo "RelayKit App gateway model list still does not respond" >&2
    return 1
  }
  provider_status="$(provider_gateway_status)"
...
write_desktop_render_evidence() {
  ...
  python3 - "${CODEX_HOME_DIR}" "${DESKTOP_RENDER_EVIDENCE}" "${since_epoch}" "${provider_model}" "${screenshot_evidence}" <<'PY'
import json
import re
import sys
from datetime import datetime
...
gpt55_source = any(model == "gpt-5.5" and "RelayKit Official 55 Live:" in text for model, text in assistant_messages)
gpt56_source = any(model == "gpt-5.6-luna" and "RelayKit Official 56 Live:" in text for model, text in assistant_messages)
...
def screenshot(role):
    for item in screenshots:
        if item.get("role") == role and item.get("captured") is True and item.get("target_identity_verified") is True:
            return item
    return {}

def visual(role, key):
    return (screenshot(role).get("visual_checks") or {}).get(key) is True
...
out_path.write_text(json.dumps({
    "gpt55_gui_visible": gpt55_source and visual("gpt55-response", "official55_response_visible"),
    "gpt56_gui_visible": gpt56_source and visual("gpt56-response", "official56_response_visible"),
...
route_outcome_from_usage_file() {
  ...
  jq -r \
    ...
    '
      ...
      elif ((["before-manual-input", "gpt55-response", "gpt56-response", "provider-markdown", "provider-tool"] - [$screenshots[0][] | select(.captured == true and .target_identity_verified == true) | .role]) | length) > 0 then "target_screenshot_evidence_missing"
      else "complete"
      end
    ' "${usage_file}"
}
  ...
  case "${role}" in
    ...
    provider-markdown)
      jq -e --arg model "${provider_model}" 'any(.[]; (.model // "") == $model and (.status // "") == "completed" and (.http_status // 0) == 200)' "${usage_file}" >/dev/null &&
        jq -e --arg role "${role}" '([.[] | select(.role == $role)] | last) as $shot | $shot.captured == true and $shot.target_identity_verified == true and $shot.visual_checks.heading_visible == true and $shot.visual_checks.numbered_items_visible == true and $shot.visual_checks.table_headers_visible == true and $shot.visual_checks.bash_code_visible == true and $shot.visual_checks.bold_conclusion_visible == true and $shot.visual_checks.raw_protocol_visible == false' "${screenshot_evidence}" >/dev/null
      ;;
    provider-tool)
      [[ -s "${tool_evidence}" ]] &&
...
write_evidence() {
  ...
  local source_snapshot_hash_after harness_hash_after source_snapshot_unchanged harness_unchanged
  local sandbox_status desktopproof_refs global_guard_passed app_screenshot_sha
  local last_route_evidence official_codex_binary
  config_after="$(file_signature "${GLOBAL_CODEX_CONFIG}")"
  auth_after="$(file_signature "${GLOBAL_CODEX_AUTH}")"
  config_hash_after="$(file_hash "${GLOBAL_CODEX_CONFIG}")"
  auth_hash_after="$(file_hash "${GLOBAL_CODEX_AUTH}")"
  ...
  jq -n \
    ...
    '{
      ...
      desktop_gui_route_proof: (
        if $route_status == "complete" and $input_mode == "manual_user_only" then "manual_user_assisted_complete"
      ...
      desktop_gui_tool_ui_review: (
        if ($tool[0].proof_found // false) and ($render[0].tool_gui_verified // false) and $input_mode == "manual_user_only" then "derived_from_current_run_rollout_and_process_bound_screenshot"
        elif ($tool[0].proof_found // false) then "rollout_verified_gui_display_not_verified"
        else "not_verified"
        end
      ),
      mock_ok_used: false,
...
guard app.activate(options: [.activateAllWindows, .activateIgnoringOtherApps]) else { exit(2) }
for _ in 0..<40 {
...
case "provider-markdown":
    checks["heading_visible"] = occurrences(normalized, "relaykitrichtextcheck") >= 2
    checks["numbered_items_visible"] = occurrences(normalized, "firstroutecheck") >= 2 && occurrences(normalized, "secondroutecheck") >= 2
    checks["table_headers_visible"] = occurrences(normalized, "status") >= 2 && occurrences(normalized, "route") >= 2 && occurrences(normalized, "ready") >= 4 && occurrences(normalized, "official") >= 2 && occurrences(normalized, "provider") >= 2
    checks["bash_code_visible"] = occurrences(normalized, "echorelaykit") >= 2
    checks["bold_conclusion_visible"] = occurrences(normalized, "relaykitformatok") >= 2
case "provider-tool":
...
capture_desktop_window() {
  ...
  local window_id path relative_path sha timestamp tmp visual_tmp marker
  window_id="$(jq -r '.window_id' "${DESKTOP_WINDOW_IDENTITY}")"
  path="${SCREENSHOT_DIR}/${role}.png"
  relative_path="${path#${ROOT}/}"
  /usr/sbin/screencapture -x -l "${window_id}" "${path}"
  [[ -s "${path}" ]] || return 1
  sha="$(/usr/bin/shasum -a 256 "${path}" | awk '{print $1}')"
  timestamp="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  marker=""
  [[ -f "${TOOL_MARKER_FILE}" ]] && marker="$(cat "${TOOL_MARKER_FILE}")"
  visual_tmp="${RUN_DIR}/desktop-visual-${role}.json"
  analyze_desktop_screenshot "${role}" "${path}" "${marker}" >"${visual_tmp}"
  jq -e 'type == "object" and .ocr_completed == true' "${visual_tmp}" >/dev/null
  ...
  jq \
    ...
    }]' "${SCREENSHOT_EVIDENCE}" >"${tmp}"
  mv "${tmp}" "${SCREENSHOT_EVIDENCE}"
  rm -f "${visual_tmp}"
}
...
EOF
  ...
  --test-preserve-route-evidence)
    [[ -n "${3:-}" ]] || exit 2
    preserve_existing_route_evidence "$2" "$3"
    ;;
  --test-route-outcome)
    [[ -n "${6:-}" ]] || exit 2
  ...
  --test-render-evidence)
    ...
    write_desktop_render_evidence "$4" "$5" "$6"
    ;;
