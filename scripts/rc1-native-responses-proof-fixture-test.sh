...
for mode in plain markdown; do
  ...
  curl -fsS -H 'Authorization: Bearer RELAYKIT_FAKE_SENTINEL_DO_NOT_USE' -H 'Content-Type: application/json' \
...
curl -fsS -N -H 'Authorization: Bearer RELAYKIT_FAKE_SENTINEL_DO_NOT_USE' -H 'Content-Type: application/json' \
